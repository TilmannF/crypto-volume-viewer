...
//! tests that need a real file `open_volume` can open, without depending on
//! VeraCrypt or any committed fixture.
//!
//! This is a trimmed, local copy of the synthetic container construction in
//! `crates/cryptovol-tcvc/tests/data_reader.rs`: cryptovol-tcvc does not
//! export this as a test-support API, and these tests need bytes written to
//! a real file (for `FileBlockReader`) rather than an in-memory reader.

...
4. **`@wdio/tauri-service@1.2.0` has a broken dependency pin.** It imports `installMockSyncOverride` from `@wdio/native-utils@2.4.0`, but that export only exists starting `2.5.0` (published a day later). Was worked around with an npm `overrides` entry in `apps/cryptovol-gui/package.json` (`"@wdio/native-utils": "2.5.0"`). **Resolved upstream; override removed (2026-09-24).** Later releases pin a `native-utils` that has the export (`tauri-service@1.3.0` → `2.6.0`, `@1.4.0` → `2.7.0`, checked with `npm view @wdio/tauri-service@<v> dependencies`). Once `tauri-service` was bumped, the leftover override became harmful: it forced `native-utils` *down* to `2.5.0`, below the exact version the service pins. The unit-test CI job does not load `@wdio/tauri-service`, so it cannot catch this; only `npm run test:e2e` does. With `tauri-service@1.4.0`, the lockfile resolves `native-utils@2.7.0`, and its ESM entry exports `installMockSyncOverride`. If a future `tauri-service` release breaks its `native-utils` pin again, prefer bumping `tauri-service` over reintroducing an override. If an override is unavoidable, remove it as soon as `tauri-service` moves.

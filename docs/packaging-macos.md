...
Hashes the single `.dmg` (and a zipped `.app`, if one exists) in `dist/macos/<version>/` with `shasum -a 256`, writing `SHA256SUMS.txt` with relative filenames. It never attempts to hash a raw `.app` directory. It fails if the folder holds no `.dmg` or more than one, or if a file name is not GitHub-safe (see "Artifact layout"). `scripts/package-macos-release.sh` calls it after stapling the DMG; `scripts/package-macos-local.sh` does not, and instead deletes any earlier `SHA256SUMS.txt` so a stale checksum file never sits next to a new DMG. It can also be run standalone against an already-built `dist/macos/<version>/`.
...
Before creating anything, the script checks that `dist/macos/<version>/` holds exactly one `.dmg`, that `SHA256SUMS.txt` lists it under a GitHub-safe name, that `shasum -a 256 -c` passes there, that the DMG has a stapled ticket, and that the release does not exist yet. `./scripts/publish-github-release.sh --dry-run` runs all of these checks and stops before creating the release.
